...
const closestIntersection = function(segment1, segment2) {
	...
	for (let point of segment1) {
		let dir = point.slice(0, 1);
		let dist = Number(point.slice(1));
		for (let i = 0; i < dist; i++) {
			[x, y] = [x + direction[dir][0], y + direction[dir][1]];
			points.add(`${x},${y}`);
		}
	}
	[x, y] = [0, 0];
	for (let point of segment2) {
		let dir = point.slice(0, 1);
		let dist = Number(point.slice(1));
		for (let i = 0; i < dist; i++) {
			[x, y] = [x + direction[dir][0], y + direction[dir][1]];
			if (points.has(`${x},${y}`)) {
				const coord = `${x},${y}`.split(',');
				if (Math.abs(coord[0]) + Math.abs(coord[1]) < closest)
					closest = Math.abs(coord[0]) + Math.abs(coord[1]);
			}
		}
	}
	return closest;
...
const minSteps = function(segment1, segment2) {
	segment1 = segment1.split(',');
	segment2 = segment2.split(',');
	let min = Infinity;
	const points = {};
	...
	for (let point of segment2) {
		...
		for (let i = 0; i < dist; i++) {
			[x, y] = [x + direction[dir][0], y + direction[dir][1]];
			if (points[`${x},${y}`]) {
				if (points[`${x},${y}`] + steps2 < min) {
					min = points[`${x},${y}`] + steps2;
				}
			}
			steps2++;
		}
...
console.log(closestIntersection(seg1, seg2));
console.log(minSteps(seg1, seg2));
